#!/usr/bin/env python3
"""Launch the frozen 18-job natural-PIV baseline and direct-NLL grid."""
...
METHODS: dict[str, dict[str, Any]] = {
    "mse": {
        "config": Path("configs/experiment/realpdebench_cylinder_mse.yaml"),
        "tag": "mse",
        "runner_method": "mse",
        "loss": "mse",
        "q": None,
    ...
    "lad": {
        "config": Path("configs/experiment/realpdebench_cylinder_lad.yaml"),
        "tag": "lad",
        "runner_method": "lad",
        "loss": "l1",
        "q": None,
    ...
    "pinn_ebm_w1": {
        "config": Path(
            "configs/experiment/"
            "realpdebench_cylinder_pinn_ebm_equal_weight.yaml"
        ),
        "tag": "pinn_ebm",
    ...
    "pinn_ebm_w10": {
        "config": Path(
            "configs/experiment/"
            "realpdebench_cylinder_pinn_ebm_weight10.yaml"
        ),
...
def validate_protocol(args: argparse.Namespace) -> Path:
    ...
    if observed_schedule != expected_schedule:
        raise ValueError(f"Baseline schedule changed: {observed_schedule}")
    if common.get("pde", {}).get("learn_reynolds") is not False:
        raise ValueError("Natural-PIV baselines require fixed Reynolds number")
    if common.get("batch") != {"n_f": 8192, "n_data": 2048}:
        raise ValueError(f"Batch protocol changed: {common.get('batch')}")

    seen_tags: set[str] = set()
    observed_direct_weights = []
    for campaign_method, expected in METHODS.items():
        config = load_yaml(expected["config"])
        if config.get("tag") != expected["tag"]:
            raise ValueError(
                f"{campaign_method} tag changed: {config.get('tag')!r}"
            )
        if config.get("method", {}).get("kind") != expected["runner_method"]:
            raise ValueError(
                f"{campaign_method} selects a different runner method"
            )
        if expected["tag"] in seen_tags:
            raise ValueError(f"Duplicate baseline tag: {expected['tag']}")
        seen_tags.add(expected["tag"])
        expected_weight = expected["joint_pde_weight"]
        configured_weight = config.get("training", {}).get(
            "joint_pde_weight"
        )
        if expected_weight is None:
            if configured_weight is not None:
                raise ValueError(
                    f"{campaign_method} unexpectedly sets a joint PDE weight"
                )
        else:
            if float(configured_weight) != float(expected_weight):
                raise ValueError(
                    f"{campaign_method} weight changed: {configured_weight}"
                )
            observed_direct_weights.append(float(configured_weight))
    if sorted(observed_direct_weights) != [1.0, 10.0, 50.0]:
        raise ValueError(
            f"Direct-NLL weight grid changed: {observed_direct_weights}"
        )
    return data_path
...
def run_name(job: dict[str, Any]) -> str:
    return (
        f"natural_piv_baseline_{job['campaign_method']}_"
        f"seed_{job['seed']}"
    )
...
def completed(root: Path, job: dict[str, Any]) -> bool:
    path = run_dir(root, job) / "metrics.json"
    if not path.is_file():
        return False
    metrics = json.loads(path.read_text(encoding="utf-8"))
    expected_init_steps = (
        5000 if job["runner_method"] == "pinn_ebm" else 0
    )
    return (
        ...
        and metrics.get("method") == job["runner_method"]
        ...
        and int(metrics.get("estimator_init_steps", -1))
        == expected_init_steps
    )
    ...
    *,
    ...
    if expected_dir.exists():
        raise FileExistsError(
            "Refusing to reuse an incomplete baseline run directory: "
            f"{expected_dir}"
        )
    log_path = (
        args.root
        / "logs"
        / (
            f"{job['campaign_method']}_seed_{job['seed']}_"
            f"cuda_{args.gpu}.log"
        )
    )
    if log_path.exists():
        raise FileExistsError(f"Refusing to overwrite job log: {log_path}")
    log_path.parent.mkdir(parents=True, exist_ok=True)
    status_path = (
        args.root
        / "status"
        / "jobs"
        / f"{job['campaign_method']}_seed_{job['seed']}.json"
    )
    command = [
        sys.executable,
        "-m",
...
def main(args: argparse.Namespace) -> None:
    ...
    status_path = (
        ...
        / "status"
        / (
            f"natural_piv_baselines_cuda_{args.gpu}_"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate the frozen protocol and print this shard without writes.",
    )
