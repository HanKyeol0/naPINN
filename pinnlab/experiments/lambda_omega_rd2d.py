...
def render_frame_worker(args):
    """
    Render Lambda-Omega RD solution: True magnitude, Pred magnitude, Noisy data, Error.
    """
    ...
    plt.suptitle(f"Lambda-Omega RD | t={t_val:.3f}", y=0.95, fontsize=14)

    mag_pred = np.sqrt(u_pred**2 + v_pred**2)
    mag_true = np.sqrt(u_true**2 + v_true**2)
    error = np.abs(mag_true - mag_pred)

    im0 = ax[0, 0].imshow(mag_true, origin="lower", extent=extent, vmin=vmin, vmax=vmax, cmap="viridis")
    ax[0, 0].set_title("True Magnitude |U*|")
    ax[0, 0].set_ylabel("y")
    plt.colorbar(im0, ax=ax[0, 0], fraction=0.046, pad=0.04)

    im1 = ax[0, 1].imshow(mag_pred, origin="lower", extent=extent, vmin=vmin, vmax=vmax, cmap="viridis")
    ...
    if X_meas_slice is not None and len(X_meas_slice) > 0:
        sc = ax[1, 0].scatter(
            ...
            vmin=vmin, vmax=vmax, cmap="viridis", s=15, edgecolors="none", alpha=0.9
        )
...
def render_noise_worker(args):
    """
    Render noise analysis for Lambda-Omega RD.
    """
...
class LambdaOmegaRD2D(BaseExperiment):
    """
    2D Lambda-Omega Reaction-Diffusion system.
    Variables: u, v
    """
    ...
    def __init__(self, cfg, device):
        ...
        print(f"[LambdaOmegaRD2D] Loading data from {data_path}")
        ...
        self.true_lambda = float(raw_data.get("lambda_val", pde_cfg.get("lambda", 1.0)))
        self.true_omega = float(raw_data.get("omega", pde_cfg.get("omega", 1.0)))

        self.learn_Du = bool(pde_cfg.get("learn_Du", False))
        self.learn_Dv = bool(pde_cfg.get("learn_Dv", False))
        self.learn_lambda = bool(pde_cfg.get("learn_lambda", False))
        self.learn_omega = bool(pde_cfg.get("learn_omega", False))
        ...
        if self.learn_Du:
            ...
            print(f"[LambdaOmegaRD2D] Du TRAINABLE. Init: {init_du}, True: {self.true_Du}")
        else:
        ...
        if self.learn_Dv:
            ...
            print(f"[LambdaOmegaRD2D] Dv TRAINABLE. Init: {init_dv}, True: {self.true_Dv}")
        else:
            self.Dv = self.true_Dv

        if self.learn_lambda:
            init_lam = float(pde_cfg.get("init_lambda", self.true_lambda))
            self.lambda_val = torch.nn.Parameter(torch.tensor(init_lam, dtype=torch.float32, device=device))
            print(f"[LambdaOmegaRD2D] lambda TRAINABLE. Init: {init_lam}, True: {self.true_lambda}")
        else:
            self.lambda_val = self.true_lambda

        if self.learn_omega:
            init_omega = float(pde_cfg.get("init_omega", self.true_omega))
            self.omega = torch.nn.Parameter(torch.tensor(init_omega, dtype=torch.float32, device=device))
            print(f"[LambdaOmegaRD2D] omega TRAINABLE. Init: {init_omega}, True: {self.true_omega}")
        else:
    ...
    def state_dict(self):
        ...
        if self.learn_Dv:
            state["Dv"] = self.Dv
        if self.learn_lambda:
            state["lambda_val"] = self.lambda_val
        if self.learn_omega:
    ...
    def load_state_dict(self, state_dict):
        if "running_std" in state_dict:
            ...
            print(f"[LambdaOmegaRD2D] Loaded running_std: {self.running_std.item():.4f}")
        ...
        if "lambda_val" in state_dict and self.learn_lambda:
            with torch.no_grad():
                self.lambda_val.copy_(state_dict["lambda_val"].to(self.device))
        if "omega" in state_dict and self.learn_omega:
            with torch.no_grad():
                self.omega.copy_(state_dict["omega"].to(self.device))
    ...
    def _init_noisy_dataset(self):
        ...
        if self.use_extra_noise:
            ...
            if n_extra > 0:
                print(f"[LambdaOmegaRD2D] Injecting outliers into {n_extra} points.")
    ...
    def pde_residual_loss(self, model, batch):
        ...
        r2 = u.pow(2) + v.pow(2)
        res_u = u_t - self.Du * (u_xx + u_yy) - (self.lambda_val - r2) * u + self.omega * v
        res_v = v_t - self.Dv * (v_xx + v_yy) - (self.lambda_val - r2) * v - self.omega * u
    ...
    def extra_params(self):
        ...
        if isinstance(self.lambda_val, torch.nn.Parameter):
            params.append(self.lambda_val)
    ...
    def relative_l2_on_grid(self, model, grid_cfg=None, batch_size=10000):
        ...
        print(f"[LambdaOmegaRD2D] Global Relative L2 Error: {rel_l2:.6f}")
        return float(rel_l2)

    def make_video(self, model, grid_cfg, out_dir, fps=10, filename="flow_evolution_lambda_omega.mp4", phase=0):
        ...
        print(f"[LambdaOmegaRD2D] Pre-calculating frames. Extent: {self.extent}")
        ...
        print(f"[LambdaOmegaRD2D] Rendering {len(render_args_list)} frames using {n_workers} workers...")
        ...
        print(f"[LambdaOmegaRD2D] Video saved to {path}")
    ...
    def _make_noise_videos(self, model, out_dir, fps, filename):
        ...
        print("[LambdaOmegaRD2D] Generating Noise Analysis video...")
        ...
        imageio.mimsave(path, frames, fps=fps, macro_block_size=None)
        print(f"[LambdaOmegaRD2D] Noise video saved to {path}")
