...
class NavierStokes2D(BaseExperiment):
    ...
    def __init__(self, cfg, device):
        ...
        self.X_data = None
        self.u_data, self.v_data, self.p_data = None, None, None
        self.u_clean, self.v_clean, self.p_clean = None, None, None
    ...
    def _init_noisy_dataset(self):
        ...
        with torch.no_grad():
            ...
            v_clean = self.v_star(X[:, 0:1], X[:, 1:2], X[:, 2:3])  # [n, 1]
            p_clean = self.p_star(X[:, 0:1], X[:, 1:2], X[:, 2:3])  # [n, 1]
        ...
        u_noisy, v_noisy, p_noisy = u_clean + eps_u, v_clean + eps_v, p_clean + eps_p
        
        self.X_data = X
        self.u_clean, self.v_clean, self.p_clean = u_clean, v_clean, p_clean
        self.u_data, self.v_data, self.p_data = u_noisy, v_noisy, p_noisy
    ...
    def sample_batch(self, n_f, n_b, n_0):
        ...
        if self.use_data and self.X_data is not None and self.n_data_batch > 0:
            ...
            batch["u_d"], batch["v_d"], batch["p_d"] = self.u_data[idx], self.v_data[idx], self.p_data[idx]
