...
from pinnlab.utils.data_loss import data_loss_l1, data_loss_mse, data_loss_q_gaussian
...
def render_frame_worker(args):
    """
    Render FitzHugh-Nagumo RD solution: True magnitude, Pred magnitude, Noisy data, Error.
    """
    ...
    plt.suptitle(f"FitzHugh-Nagumo RD | t={t_val:.3f}", y=0.95, fontsize=14)

    mag_pred = np.sqrt(u_pred**2 + v_pred**2)
    mag_true = np.sqrt(u_true**2 + v_true**2)
    error = np.abs(mag_true - mag_pred)

    im0 = ax[0, 0].imshow(mag_true, origin="lower", extent=extent, vmin=vmin, vmax=vmax, cmap="magma")
    ax[0, 0].set_title("True Magnitude |U*|")
    ax[0, 0].set_ylabel("y")
    plt.colorbar(im0, ax=ax[0, 0], fraction=0.046, pad=0.04)

    im1 = ax[0, 1].imshow(mag_pred, origin="lower", extent=extent, vmin=vmin, vmax=vmax, cmap="magma")
    ax[0, 1].set_title("Predicted Magnitude |U_hat|")
    ...
    if X_meas_slice is not None and len(X_meas_slice) > 0:
        sc = ax[1, 0].scatter(
            X_meas_slice[:, 0], X_meas_slice[:, 1], c=mag_meas_slice,
            vmin=vmin, vmax=vmax, cmap="magma", s=15, edgecolors="none", alpha=0.9
        )
    ...
    ax[1, 1].set_title("Absolute Error |U* - U_hat|")
    ax[1, 1].set_xlabel("x")
...
def render_noise_worker(args):
    """
    Render noise analysis for FitzHugh-Nagumo RD.
    """
    ...
    plt.suptitle(f"Noise Analysis | t={t_val:.3f}", y=0.96, fontsize=14)

    cmap = "coolwarm"
    vm = R_range

    im0 = axes[0, 0].imshow(eps_u_grid, origin="lower", extent=extent, vmin=-vm, vmax=vm, cmap=cmap)
    axes[0, 0].set_title("True Noise Field epsilon_u(x,y)")
    axes[0, 0].set_ylabel("y")
    fig.colorbar(im0, ax=axes[0, 0], fraction=0.046, pad=0.04)

    im1 = axes[0, 1].imshow(res_u_grid, origin="lower", extent=extent, vmin=-vm, vmax=vm, cmap=cmap)
    axes[0, 1].set_title("Residual Field r_u = y_u - u_hat")
...
class FitzHughNagumoRD2D(BaseExperiment):
    """
    2D FitzHugh-Nagumo Reaction-Diffusion system.
    Variables: u, v
    """
    ...
    def __init__(self, cfg, device):
        ...
        print(f"[FitzHughNagumoRD2D] Loading data from {data_path}")
        raw_data = np.load(data_path)

        pde_cfg = cfg.get("pde", {}) or {}
        self.true_Du = float(raw_data.get("Du", pde_cfg.get("Du", 1.0)))
        self.true_Dv = float(raw_data.get("Dv", pde_cfg.get("Dv", 1.0)))
        self.true_a = float(raw_data.get("a", pde_cfg.get("a", 0.7)))
        self.true_b = float(raw_data.get("b", pde_cfg.get("b", 0.8)))
        self.true_eps = float(raw_data.get("epsilon", pde_cfg.get("epsilon", 0.08)))
        self.true_I = float(raw_data.get("I", pde_cfg.get("I", 0.5)))

        self.learn_Du = bool(pde_cfg.get("learn_Du", False))
        self.learn_Dv = bool(pde_cfg.get("learn_Dv", False))
        self.learn_a = bool(pde_cfg.get("learn_a", False))
        self.learn_b = bool(pde_cfg.get("learn_b", False))
        self.learn_eps = bool(pde_cfg.get("learn_epsilon", False))
        self.learn_I = bool(pde_cfg.get("learn_I", False))

        if self.learn_Du:
            init_du = float(pde_cfg.get("init_Du", self.true_Du))
            self.Du = torch.nn.Parameter(torch.tensor(init_du, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] Du TRAINABLE. Init: {init_du}, True: {self.true_Du}")
        else:
            self.Du = self.true_Du

        if self.learn_Dv:
            init_dv = float(pde_cfg.get("init_Dv", self.true_Dv))
            self.Dv = torch.nn.Parameter(torch.tensor(init_dv, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] Dv TRAINABLE. Init: {init_dv}, True: {self.true_Dv}")
        else:
            self.Dv = self.true_Dv

        if self.learn_a:
            init_a = float(pde_cfg.get("init_a", self.true_a))
            self.a = torch.nn.Parameter(torch.tensor(init_a, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] a TRAINABLE. Init: {init_a}, True: {self.true_a}")
        else:
            self.a = self.true_a

        if self.learn_b:
            init_b = float(pde_cfg.get("init_b", self.true_b))
            self.b = torch.nn.Parameter(torch.tensor(init_b, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] b TRAINABLE. Init: {init_b}, True: {self.true_b}")
        else:
            self.b = self.true_b

        if self.learn_eps:
            init_eps = float(pde_cfg.get("init_epsilon", self.true_eps))
            self.epsilon = torch.nn.Parameter(torch.tensor(init_eps, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] epsilon TRAINABLE. Init: {init_eps}, True: {self.true_eps}")
        else:
            self.epsilon = self.true_eps

        if self.learn_I:
            init_I = float(pde_cfg.get("init_I", self.true_I))
            self.I = torch.nn.Parameter(torch.tensor(init_I, dtype=torch.float32, device=device))
            print(f"[FitzHughNagumoRD2D] I TRAINABLE. Init: {init_I}, True: {self.true_I}")
        else:
            self.I = self.true_I

        self.X_f_all = torch.from_numpy(raw_data["X_f"]).float().to(device)
        self.X_u_clean = torch.from_numpy(raw_data["X_u"]).float().to(device)
        self.Y_u_clean = torch.from_numpy(raw_data["Y_u"]).float().to(device)
        ...
        self.ebm_kind = ebm_cfg.get("kind", "1D")
    ...
    def load_state_dict(self, state_dict):
        if "running_std" in state_dict:
            ...
            print(f"[FitzHughNagumoRD2D] Loaded running_std: {self.running_std.item():.4f}")

        if "offset" in state_dict and self.offset is not None:
            with torch.no_grad():
                self.offset.copy_(state_dict["offset"].to(self.device))

        if "Du" in state_dict and self.learn_Du:
            with torch.no_grad():
                self.Du.copy_(state_dict["Du"].to(self.device))
        if "Dv" in state_dict and self.learn_Dv:
            with torch.no_grad():
                self.Dv.copy_(state_dict["Dv"].to(self.device))
        if "a" in state_dict and self.learn_a:
            with torch.no_grad():
                self.a.copy_(state_dict["a"].to(self.device))
        if "b" in state_dict and self.learn_b:
            with torch.no_grad():
                self.b.copy_(state_dict["b"].to(self.device))
        if "epsilon" in state_dict and self.learn_eps:
            with torch.no_grad():
                self.epsilon.copy_(state_dict["epsilon"].to(self.device))
        if "I" in state_dict and self.learn_I:
            with torch.no_grad():
                self.I.copy_(state_dict["I"].to(self.device))
    ...
    def _init_noisy_dataset(self):
        ...
        if self.use_extra_noise:
            ...
            if n_extra > 0:
                print(f"[FitzHughNagumoRD2D] Injecting outliers into {n_extra} points.")
    ...
    def pde_residual_loss(self, model, batch):
        ...
        res_u = u_t - self.Du * (u_xx + u_yy) - u + u**3 + v - self.I
        res_v = v_t - self.Dv * (v_xx + v_yy) - self.epsilon * (u - self.a * v - self.b)
    ...
    def data_loss(self, model, batch, phase=1):
        ...
        if self.ebm_kind == "1D":
            data_loss_value = data_loss_value.view(-1, 1)
    ...
    def _data_loss(self, residual):
        ...
        if self.data_loss_kind == "L1":
            return data_loss_l1(residual)
        if self.data_loss_kind == "q_gaussian":
    ...
    def extra_params(self):
        ...
        if isinstance(self.Du, torch.nn.Parameter):
            params.append(self.Du)
        if isinstance(self.Dv, torch.nn.Parameter):
            params.append(self.Dv)
        if isinstance(self.a, torch.nn.Parameter):
            params.append(self.a)
        if isinstance(self.b, torch.nn.Parameter):
            params.append(self.b)
        if isinstance(self.epsilon, torch.nn.Parameter):
            params.append(self.epsilon)
        if isinstance(self.I, torch.nn.Parameter):
            params.append(self.I)
    ...
    def relative_l2_on_grid(self, model, grid_cfg=None, batch_size=10000):
        ...
        print(f"[FitzHughNagumoRD2D] Global Relative L2 Error: {rel_l2:.6f}")
        return float(rel_l2)

    def make_video(self, model, grid_cfg, out_dir, fps=10, filename="flow_evolution_fitzhugh_nagumo.mp4", phase=0):
        ...
        print(f"[FitzHughNagumoRD2D] Pre-calculating frames. Extent: {self.extent}")
        ...
        with torch.no_grad():
            for i, t_val in enumerate(self.val_t):
                ...
                if self.use_data and self.X_data is not None:
                    ...
                    mask_time = (X_d_cpu[:, 2] >= t_val - dt_window) & (X_d_cpu[:, 2] < t_val + dt_window)
        ...
        print(f"[FitzHughNagumoRD2D] Rendering {len(render_args_list)} frames using {n_workers} workers...")
        ...
        print(f"[FitzHughNagumoRD2D] Video saved to {path}")

        if phase == 2:
            try:
                self._make_noise_videos(model, out_dir, fps, filename)
            except Exception as exc:
                print(f"Warning: Failed to create noise analysis video: {exc}")
                traceback.print_exc()
    ...
    def _make_noise_videos(self, model, out_dir, fps, filename):
        ...
        print("[FitzHughNagumoRD2D] Generating Noise Analysis video...")
        ...
        with torch.no_grad():
            ...
            log_q = self.ebm(r_input_scaled).squeeze(-1)
            m = log_q.max()
        ...
        imageio.mimsave(path, frames, fps=fps, macro_block_size=None)
        print(f"[FitzHughNagumoRD2D] Noise video saved to {path}")
    ...
    def evaluate_gate_performance(self, model, out_dir, filename_prefix=None):
        ...
        with torch.no_grad():
            ...
            log_q = self.ebm(res_scaled).squeeze(-1)
        ...
        ax.plot(z_grid, w_curve, "k--", linewidth=2, label=f"Learned Gate (alpha={alpha:.2f}, beta={beta:.2f})")
