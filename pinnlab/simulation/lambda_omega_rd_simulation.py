...
def main(args):
    ...
    LAMBDA = float(pde_cfg.get("lambda", 1.0))
    OMEGA = float(pde_cfg.get("omega", 1.0))
    ...
    def solve_lambda_omega():
        ...
        u = 0.1 * generate_grf(X, Y, alpha=5.0, seed=1)
        v = 0.1 * generate_grf(X, Y, alpha=5.0, seed=2)
        ...
        for n in tqdm(range(total_steps)):
            ...
            r2 = u**2 + v**2
            du_dt = DU * lap_u + (LAMBDA - r2) * u - OMEGA * v
            dv_dt = DV * lap_v + (LAMBDA - r2) * v + OMEGA * u
    ...
    def save_visualizations(u_full, v_full, t_grid, x_grid, y_grid, X_u):
        ...
        cax = ax.pcolormesh(X, Y, mag[0], shading="auto", cmap="viridis", vmin=vmin, vmax=vmax)
        ...
        title = ax.set_title(f"Lambda-Omega RD t={t_grid[0]:.3f}")

        def update(frame_idx):
            t_current = t_grid[frame_idx]
            cax.set_array(mag[frame_idx].ravel())
            title.set_text(f"Lambda-Omega RD t={t_current:.3f}")
    ...
    u_h, v_h, t_h, x_g, y_g, x_c, y_c = solve_lambda_omega()
